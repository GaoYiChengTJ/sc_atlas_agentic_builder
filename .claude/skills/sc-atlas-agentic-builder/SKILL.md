...
Install the package from GitHub before use:
```bash
...
```python
# If installed via pip install -e . (recommended), no sys.path manipulation needed.
# If running from the repo root without installing, add the repo parent to sys.path:
# import sys; sys.path.insert(0, "/path/to/parent/of/sc_atlas_agentic_builder")
